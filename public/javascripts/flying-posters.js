// This function will be called by the script in layout.jade
function initFlyingPosters(canvasElement, items = []) {
  // Use the global 'ogl' object from the UMD script
  ...
  class Media {
    ...
    onResize({ screen, viewport } = {}) {
      ...
      this.plane.position.x = 0; // Ensure x position is reset
    }
  ...
  class Canvas {
    constructor({ canvas, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ }) {
      this.canvas = canvas;
      this.container = this.canvas.parentElement; // Get container from canvas
      this.items = items;
      this.planeWidth = planeWidth;
      this.planeHeight = planeHeight;
      this.distortion = distortion;
      ...
      this.createCamera();
      this.createScene();
      this.onResize(); // Call onResize immediately
      ...
      this.items.forEach(src => {
        ...
        image.onload = () => {
          ...
          if (this.loaded === this.items.length) {
            // You can add a 'loaded' class if you want
    ...
    addEventListeners() {
      ...
      // Add wheel/touch events directly to the canvas
      ...
      this.canvas.addEventListener('mouseleave', this.onTouchUp); // End drag if mouse leaves
    ...
    // Note: No destroy method is provided, but in a real app you'd
    // want one to remove listeners if you navigate away.
  }
  ...
  new Canvas({
    ...
    planeHeight: 320,
    distortion: 3,
    scrollEase: 0.05, // Adjusted ease for smoother scroll
    cameraFov: 45,
    cameraZ: 20
  });
}
